...
interface BayanAppGeneratorProps {
  ...
  onSaveToFiles?: (appName: string, code: string) => void;
}
...
export const BayanAppGenerator: React.FC<BayanAppGeneratorProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    const steps = [
      { text: `🔮 [الذكاء الصرف] جاري تحليل واقتراح البنية التحتية لتطبيق "${options.appName}"...`, duration: 600 },
      { text: `📐 [محلل الهياكل] مطابقة الكيانات البرمجية وتخطيط واجهة (${options.uiStyle === 'cards' ? 'البطاقات الحديثة' : options.uiStyle === 'royal' ? 'الملكية الفاخرة' : 'الهيكلية الدقيقة'})...`, duration: 500 },
      { text: `🎨 [المعالج اللوني] توليد لوحة الألوان وتجهيز أكواد النمط السداسي عشر لثيم (${options.colorTheme === 'emerald' ? 'رونق الزمرد' : options.colorTheme === 'gold' ? 'بريق الذهب' : 'الفضاء السديمي'})...`, duration: 500 },
      { text: `💻 [مصمم العتاد] تجميع واجهات الاستدعاء السريع (${options.enableLocalDb ? 'قاعدة بيانات SQLite المحلية' : ''} ${options.enableVoice ? '، ومستشعرات الصوت والـ Audio' : ''})...`, duration: 600 },
      { text: `⚙️ [مترجم الأندرويد والويب] ربط توثيقات الأندرويد والـ PWA السريعة لضمان تصفح مثالي دون اتصال...`, duration: 600 },
      { text: `🚀 [السيادة الرقمية] تجميع الشيفرة البرمجية متكاملة الأطراف (Bayan Compiled Code) ونسب انبعاث طاقة صفرية...`, duration: 700 },
      { text: `💾 [نظام الملفات الموطن] حفظ التطبيق آليًا في المجلدات والملفات: "التطبيقات المولدة/${options.appName}.byn"...`, duration: 700 }
    ];
    ...
    // Call onSaveToFiles to automatically store inside the virtual project folders and files
    if (onSaveToFiles) {
      onSaveToFiles(options.appName, generatedBayanCode);
    }
    ...
    setTimeout(() => {
      ...
      const startLogs = [
        ...
        `[${bootTime}] 💾 تم الحفظ التلقائي بنجاح في مجلد "التطبيقات المولدة/${options.appName}.byn" 📂`
      ];
  ...
  const getCustomBayanCode = (opt: GenerationOptions) => {
    ...
    if (opt.enableLocalDb) {
      dbCode = `
    // [٣] الكيان التخزيني وقاعدة البيانات المحلية - SQLite
    حاول:
        ...
        قاعدة_بيانات.تحديث_أو_إضافة("حالة_التكامل"، "متوافق_عالمياً"، "نعم")
        اطبع("💾 [قاعدة البيانات] تم تسجيل مستودع SQLite الموطن بنجاح!")
    التقط (خطأ_التخزين):
        اطبع("⚠️ خطأ أثناء تهيئة مستودع البيانات: " + خطأ_التخزين)
    نهاية
`;
    }
...
`;
    ...
    if (opt.enableHaptic) {
      hapticCode = `
        // الاهتزاز النبضي المرتد لتأكيد التفاعل الفيزيائي
...
`;
    ...
    if (opt.enableQuantum) {
      quantumCode = `
    // [٥] منظومة التسريع والحساب التوازني الكمي O(1)
    حاول:
        ...
        اطبع("📡 طفرة المعالجة السيادية الكمية: تراكب قمة الحالة (" + قياس_السيادة + ") بنجاح!")
    التقط (خطأ_الكم):
        اطبع("⚠️ تعذر بدء المعالج الكمي الافتراضي: " + خطأ_الكم)
    نهاية
`;
    }
    ...
    if (opt.enableNeuralNet) {
      neuralCode = `
    // [٦] الشبكة العصبية الموطنة والذكاء التطوري الجيني
    حاول:
    ...
    التقط (خطأ_عصبي):
        اطبع("⚠️ تعذر بناء النموذج التطوري: " + خطأ_عصبي)
    نهاية
`;
    }
    ...
    أندرويد.زر("زر_الدخول"، "✨ دخول البوابة الملكية الاستراتيجية")
`;
    ...
    أندرويد.عنوان_رئيسي("💻 ${opt.appName}")
    أندرويد.زر("زر_الدخول"، "⌨️ تشغيل محطة التحكم المركزية")
`;
    } else if (opt.uiStyle === 'minimal') {
      layoutElements = `
    أندرويد.إضافة_واجهة("الواجهة_البسيطة_الناصعة")
    أندرويد.نمط_الواجهة("مظهر_أبيض_بسيط")
    أندرويد.عنوان_رئيسي("🍃 ${opt.appName}")
    أندرويد.زر("زر_الدخول"، "🚶‍♂️ ابدأ الآن")
`;
    } else if (opt.uiStyle === 'sand') {
      layoutElements = `
    أندرويد.إضافة_واجهة("واجهة_الرمال_الدافئة")
    أندرويد.نمط_الواجهة("مظهر_صحراوي_أصيل")
    أندرويد.عنوان_رئيسي("🏜️ ${opt.appName}")
    أندرويد.زر("زر_الدخول"، "🧭 استكشاف المعالم والبيانات")
`;
    } else {
      layoutElements = `
    ...
    أندرويد.عنوان_رئيسي("📱 ${opt.appName}")
    أندرويد.زر("زر_الدخول"، "🚪 دخول لوحة التحكم الرئيسية")
`;
    }

    return `// =========================================================================
// 🚀 ترويسة التطوير للغة البيان (Al-Bayan Standard Specification v2.6)
// المطور المسؤول: \${opt.developerName}
// معرّف الحزمة: com.bayan.generated.\${Math.floor(Math.random() * 90000 + 10000)}
// عنوان المشروع: \${opt.appName}
// نظام التشغيل المخطط: \${isAndroid ? 'Android Native (385KB Lightweight APK)' : isWeb ? 'Independent PWA (Progressive Web App)' : 'Universal Hybrid Engine'}
// =========================================================================

// =========================================================================
// 🌐 ميثاق وقواعد لغة البيان البرمجية للتوليد القياسي المتكامل ومتوافق عالمياً:
// 
// ١. كفاءة الذاكرة الخلوية (0% Leakage):
//    تلتزم لغة البيان بإلغاء تسريبات الذاكرة تماماً عبر تفعيل التنظيف التلقائي
//    باستدعاء: أندرويد.تنظيف_ذاكرة_تلقائي() في نهاية وبداية مهام المعالجة.
//
// ٢. ثنائية البنية والترجمة العابرة للمنصات (Bicultural Portability):
//    الأكواد الناتجة تتطابق مع أنظمة أندرويد وويب لتصنيع تطبيقات فائقة الخفة (<385KB)
//    ومواقع تقدمية (PWA) غاية في الأمان تعمل بكفاءة تامة دون اتصال بالشبكة.
//
// ٣. معيار الحماية الاستثنائية والصلابة الهيكلية:
//    يجب تغليف العمليات الحساسة (التخزين، الذكاء، الاتصال) داخل جدران استثناءات 
//    صلبة باستخدام (حاول... التقط... نهاية) لضمان عدم توقف واجهة المستخدم تحت أي ظرف.
//
// ٤. تصميم وتجربة المستخدم العالمية (Universal Human Interface Guidelines):
//    - استخدام خطوط متناسقة ومقروءة (مثل Inter و Space Grotesk) مهيأة للقراءة من اليمين لليسار والعكس.
//    - تفعيل مستشعرات العتاد اللمسية والصوتية لإرساء تجربة مدمجة وغامرة.
//    - دعم الوصول السهل والشامل لذوي الاحتياجات (Accessibility Metadata).
// =========================================================================

// استيراد المكتبات الأساسية المعيارية لضمان الاستقرار والتكامل الهيكلي
استورد("رياضيات")
استورد("أجهزة_موطنة")
استورد("محرك_الصوتيات")

مهمة رئيسية():
    اطبع("🌍 جاري تحميل البنية الأساسية لتطبيق: \${opt.appName} ...")
    
    // [١] تفعيل معايير التوطين وتنظيف المسارات الخلوية لمنع الهدر اللحظي للذاكرة
    أندرويد.تنظيف_ذاكرة_تلقائي()
    
    // [٢] تهيئة تطبيق البيان الموطن بالهوية اللونية والبصرية المختارة
    أندرويد.صناعة_تطبيق("com.bayan.generated.\${Math.floor(Math.random() * 90000 + 10000)}", "\${opt.appName}")
    أندرويد.لوح_الألوان("\${opt.colorTheme === 'emerald' ? 'زمردي_فاخر' : opt.colorTheme === 'gold' ? 'ذهبي_ملكي' : opt.colorTheme === 'rose' ? 'عبير_الورد' : opt.colorTheme === 'cosmic' ? 'فضاء_سديمي' : 'نسيج_الياقوت'}")
    
    // شحن مستشعرات التسريع للمحركات المتقدمة
    أندرويد.محرك_كمومي("تألق_طاقة_مستدام")

    // بناء واجهة المستخدم المتجاوبة عالمياً وتوزيع العناصر الهيكلية
    \${layoutElements}

    // تفاصيل وتوصيف المنظومة الذكية (معلومات الوصول وميتا البيانات)
    أندرويد.نص("توصيف_البرنامج"، "📝 \${opt.description}")
    أندرويد.نص("المسؤول_التقني"، "👤 المطور المعتمد: \${opt.developerName}")
    أندرويد.مؤشر_تقدم("كفاءة_الاستهلاك_الخلوي"، ٩٩)
    ...
    \${isAndroid ? \`
    // [٧] تكوين تصاريح الوصول وتكامل عتاد أندرويد لخدمات الهاتف
    حاول:
        أندرويد.رخصة_الوصول("الاهتزاز")
        أندرويد.رخصة_الوصول("الإنترنت_المحلي")
        أندرويد.رخصة_الوصول("الوصول_للذاكرة_الآمنة")
        أندرويد.نص_توضيحي("إرشاد_النظام"، "📱 هذا التطبيق جاهز وموافق لمتطلبات متجر Google Play مع كفاءة الصفر الخلوي.")
    التقط (خطأ_العتاد):
        اطبع("⚠️ تعذر استصدار تراخيص أندرويد: " + خطأ_العتاد)
    نهاية
    \` : ''}

    \${isWeb ? \`
    // [٧] تفعيل بروتوكولات وتراخيص موقع الويب التقدمي المستقل PWA
    حاول:
        أندرويد.تهيئة_موقع_ويب("https://bayan.studio/\${opt.appName.replace(/\\s+/g, '_')}")
        أندرويد.رخصة_الوصول("التخزين_المؤقت_للخدمة")
        أندرويد.نص_توضيحي("إرشاد_الويب"، "🌐 هذا الموقع مهيأ بالكامل ليعمل كـ PWA مع تنزيل مسبق للملفات بنسبة 100%.")
    التقط (خطأ_الويب):
        اطبع("⚠️ تعذر تهيئة تراخيص الويب التقدمية: " + خطأ_الويب)
    نهاية
    \` : ''}

    // [٨] معالجة الأحداث التفاعلية ولحظات تفاعل المستخدم مع عناصر الواجهة بنمط عالمي متكامل
    تعلم.عند_النقر("زر_الدخول"):
        \${hapticCode}
        اطبع("🎯 [تفاعل المستخدم] تم رصد وتأكيد النقر على زر الدخول المركزي بنجاح!")
        أندرويد.رسالة_منبثقة("مرحباً بك في \${opt.appName}! تم التشغيل بأقصى كفاءة وموثوقية عالمية.")
    نهاية

    // [٩] تجميع وبناء ملف التوزيع والتشغيل النهائي للمشروع
    حاول:
        أندرويد.بناء_APK()
        أندرويد.تنظيف_ذاكرة_تلقائي()
        اطبع("🏁 [بناء] اكتمل تجميع وبناء التطبيق والملفات بنجاح 100% متوافقاً مع النظم والواجهات العالمية.")
    التقط (خطأ_البناء):
        اطبع("⚠️ خطأ في عملية التجميع النهائية: " + خطأ_البناء)
    نهاية
نهاية
`;
  };
              ...
              {isGenerating && (
                <div className="flex-1 flex flex-col justify-center items-center p-8 space-y-6 bg-slate-950 overflow-y-auto">
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    <p className="text-xs text-sky-400 font-bold">جاري تخليق منظومة السيادة في {genStep}/7 خطوات</p>
                  </div>
